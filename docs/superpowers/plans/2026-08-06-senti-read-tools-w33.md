...
- **No Active Context block.** This plan was written before [CONTEXT D7](../../CONTEXT.md) and originally ended each story with "refresh the `CLAUDE.md` Active Context block". Those steps are struck: the block is gone, `.active-context.md` is not created, and `docs/sprints/STATUS.md` — regenerated by `npm run agile:status`, which every close step already runs — is the only in-flight snapshot.
...
12. `CONTEXT.md` gains D8 (directory structure), D9 (`registerReadTool`), D10 (payload-shaping policy).
...
repetition D9 exists to remove, so it is extracted here rather than copied six times.
...
- Modify: `docs/sprints/stories/US-2.4-tool-substrate-and-layout.md`, `docs/sprints/sprint-2026-W33.md`, `docs/sprints/STATUS.md` (generated)
...
Expected: `D6`. Append `D8`, `D9`, `D10` under a new `## Phase 3 — Read-tool expansion (2026-08-06)` heading, each with the repo's Context / Decision / Rationale / Alternatives considered / Impact / Date / Version shape:

- **D8 — `core/` and `tools/<tag>/` replace the flat layout.** Reverses the AGENTS.md rule written for six source files, now at sixteen. Note the three anchors: recursive tsconfig globs, `src/index.ts` immovable, `test:smoke` hardcodes its path.
- **D9 — `registerReadTool`, not a descriptor table.** The v1 spec deferred this to "revisit when the repetition is real". It is real, and it is the mechanical `try`/`catch` — not the descriptions and schemas, which are what decide whether a model picks the right tool and which a table would swallow into data. Record that the annotations became constants, which makes the helper a barrier against a write tool landing before EPIC-3.
- **D10 — tools bound and shape their own payloads.** Record the alternative rejected (mirror the API, trust the host's context window) and why it fails: a year-long `breakdowns` window is roughly 70k tokens. Record that every cut leaves a `notes` trace.
...
In `sprint-2026-W33.md`, mark US-2.4 done in the scope table. `npm run agile:status` in Step 6 regenerates `STATUS.md` from that. No Active Context block is written (Global Constraints, [CONTEXT D7](../../CONTEXT.md)).
...
are built into. Records D8 (layout), D9 (registerReadTool over a descriptor
table) and D10 (payload shaping)."
```
...
- `sprint-2026-W33.md` → US-2.5 done in the scope table; `agile:status` regenerates `STATUS.md`
...
- `sprint-2026-W33.md` → US-2.6 done in the scope table; `agile:status` regenerates `STATUS.md`
...
- `sprint-2026-W33.md` → US-2.7 done in the scope table; `agile:status` regenerates `STATUS.md`
...
- `sprint-2026-W33.md` → US-2.8 done in the scope table; `agile:status` regenerates `STATUS.md`
...
- `docs/sprints/STATUS.md` → regenerated by `agile:status`; it is the only in-flight snapshot ([CONTEXT D7](../../CONTEXT.md))
...
- **Whether `capPositions`/`capOrders` generalise.** If the two cap helpers turn out identical apart from a noun, W34's first task is extracting one — but only then, on the same "revisit when the repetition is real" principle that produced `registerReadTool` rather than a descriptor table.
  ...
  six copies of purely mechanical control flow is exactly the case D9 describes. The
