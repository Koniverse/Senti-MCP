...
- [ ] **AC-2** — **Given** a well-formed draft, **When** the tool returns, **Then**
  `sourceCode`, `lastCompileLog`, `logTruncated` and `lastCompileDiagnostics` are all present
  and unmodified.
- [ ] **AC-3** — **Given** a draft with attachments, **When** the tool returns, **Then** no
...
- [ ] **AC-4** — **Given** attachment source containing non-ASCII characters, **When**
  `sourceBytes` is computed, **Then** it is the UTF-8 byte length, not the UTF-16 code-unit
  count.
- [ ] **AC-5** — **Given** a draft with at least one attachment, **When** the tool returns,
  **Then** `notes` carries one entry naming `list_draft_attachments` and the `draftId`,
  **And** the same sentence appears in `content`.
- [ ] **AC-6** — **Given** a draft with no attachments, **When** the tool returns, **Then**
  `notes` is the empty array. A note is never emitted for a cut that did not happen.
- [ ] **AC-7** — **Given** a `lastCompileDiagnostics` element matching the compile response's
...
- [ ] **AC-8** — **Given** a `lastCompileDiagnostics` element of an unrecognised shape,
  **When** the tool runs, **Then** the read succeeds, **And** the element is rendered as raw
  output rather than dropped or thrown over.
- [ ] **AC-9** — **Given** `lastCompileStatus: 'SUCCESS'` and `compiledUpToDate: true`,
...
- [ ] **AC-10** — **Given** `logTruncated: true`, **When** the text is rendered, **Then** it
  says the log is the tail only.
- [ ] **AC-11** — **Given** `lastCompileStatus: null` on a never-compiled draft, **When** the
  tool runs, **Then** it parses successfully and the text reads "never compiled" rather than
  printing `null`.
- [ ] **AC-12** — **Given** a `404` from the API, **When** the tool returns, **Then**
  `isError` is true and the text carries `DRAFT_NOT_FOUND`'s guidance, which names
  `list_drafts` and does not blame the account.

## Tasks

- [ ] **TASK-7.2.1** — **Check the contract against the live service before writing code**
  (AC: 2, 7, 8, 11)
  - [ ] Read one draft live; record its key set and compare it field-for-field against the
  ...
  - [ ] **Try to observe one real diagnostic.** Arrange a `FAILED` draft **by hand in the web
        ...
        §Implementation notes and compare it to `DiagnosticSchema`
...
- [ ] **TASK-7.2.2** — `src/tools/authoring/get-draft.ts` domain module (AC: 2–11)
  - [ ] `AttachmentSchema`, `DraftSchema`, `DiagnosticSchema`, `AttachmentSummarySchema`,
        `DraftOutputSchema`, and the `byteLength` helper the two sibling stories import
  - [ ] `parseDraft` via `parseOrThrow` with subject `draft`; `shapeDraft`; `formatDraft`
  - [ ] `formatDraft` renders diagnostics via `safeParse` with a raw fallback, states
        register-readiness, and marks a truncated log
- [ ] **TASK-7.2.3** — Registration and the `2.2.0` release (AC: 1, 12)
  - [ ] Register through `registerReadTool`; path via `draftPath(args.draftId)`;
        `scope: 'authoring:read'`; `notFoundMeans: DRAFT_NOT_FOUND`; no `conflictMeans`
  - [ ] Tool description states the response can reach ~48,000 tokens of source, that
        attachment source is **not** included, and that `list_drafts` is the cheap overview
  - [ ] `src/server.ts` registration; `TOOL_CALLS` row **with `arguments`** in
        `src/server.test.ts`
  - [ ] `VERSION`, `package.json`, `src/config.ts` `SERVER_VERSION` → `2.2.0` in lockstep;
        `docs/CHANGELOG.md` `## [2.2.0]`; `README.md` tool-table row; `AGENTS.md` tool count
        11 → 12
- [ ] **TASK-7.2.4** — Extend `src/smoke.test.ts` with a `get_draft` leg, guarded on the
