...
| `DEPLOY.md` | Nothing is deployed at any shipped version so far — the server runs from a local build wired into an MCP client by absolute path, which [SETUP.md](SETUP.md) §6 covers. It lands when there is a hosted or published artifact to describe. |
