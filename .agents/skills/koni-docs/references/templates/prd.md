...
**Contents**: [1. Heading convention (canonical — must be followed)](#1-heading-convention-canonical--must-be-followed) · [2. Full template skeleton](#2-full-template-skeleton) · [3. Updating PRD.md](#3-updating-prdmd) · [4. Functional Requirements — row format](#4-functional-requirements--row-format) · [5. Story entry in Epics & User Stories](#5-story-entry-in-epics--user-stories) · [6. Epics & Stories Index](#6-epics--stories-index) · [7. Filled example (Executive Summary + Epics & User Stories snippet)](#7-filled-example-executive-summary--epics--user-stories-snippet) · [8. Migrating an older PRD with numbered headings](#8-migrating-an-older-prd-with-numbered-headings)
