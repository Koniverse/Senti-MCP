...
- Watch: the version string lives in **three** places — `VERSION`, `package.json`, `SERVER_VERSION` in `src/config.ts`. koni-docs checks the first two; `config.test.ts` fails the suite if the third drifts. `src/` exercises **four** read scopes today (`accounts:read`, `brokers:read`, `strategies:read`, `trading:read`); `performance:read` isn't used by any shipped tool yet — it belongs to W34. Create a key with all five up front anyway, since scopes are fixed at creation and there is no key-introspection endpoint — a missing one surfaces only as a 403 naming it when the affected tool is first called. Adding an env var needs `docs/SETUP.md` + `.env.example` in the same commit (RULE-11)
