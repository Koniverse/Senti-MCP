...
- Active Stories: ✅ US-2.4 tool substrate and directory layout — closed, ships 0.2.0. ✅ US-2.5 `list_brokers` — closed, ships 0.3.0. ✅ US-2.6 `list_strategies` — closed, ships 0.4.0. ✅ US-2.7 `list_account_strategies` — closed, ships 0.5.0. ✅ US-2.8 `list_positions` — closed, ships 0.6.0. Open: US-2.9 `list_pending_orders`
- Next Up: US-2.9 `list_pending_orders` — the second of the terminal-backed pair, reusing the `409`/`conflictMeans` and "empty is a real zero" pattern US-2.8 established rather than re-deriving it, per the [read-tool expansion spec](docs/superpowers/specs/2026-08-05-senti-read-tools-expansion-design.md). Reuses `accountPath`, proven by US-2.7. Its close is this sprint's close. Four more read operations (`get_account_performance`, `get_performance_breakdowns`, `get_equity_timeseries`, `list_deals`) carry to sprint W34 (EPIC-2 stays `in-progress`)
- Last Version: 0.6.0 — the first tool where "empty" and "unavailable" are different answers. `list_positions` reads `GET /api/v1/accounts/{accountId}/positions` and returns the positions currently open on one MT5 account, read live from its MT5 terminal. A `200` with zero positions is a real zero — the terminal answered and the account holds nothing. A `409` means the terminal is offline, so the API cannot say what is held; the tool reports this as an error whose text states outright that it is "NOT the same as the account holding no positions," using the endpoint's own `conflictMeans` text (US-2.4). Routes through `accountPath`, same as US-2.7. Registers through `registerReadTool` under `trading:read`
