...
- Active Stories: ✅ US-2.4 tool substrate and directory layout — closed, ships 0.2.0. ✅ US-2.5 `list_brokers` — closed, ships 0.3.0. ✅ US-2.6 `list_strategies` — closed, ships 0.4.0. ✅ US-2.7 `list_account_strategies` — closed, ships 0.5.0. Open: US-2.8 `list_positions`, US-2.9 `list_pending_orders`
- Next Up: US-2.8 `list_positions` — the first of the terminal-backed pair, carrying the `409` terminal-offline branch and the "empty is a real zero" distinction, per the [read-tool expansion spec](docs/superpowers/specs/2026-08-05-senti-read-tools-expansion-design.md). Reuses `accountPath`, proven by US-2.7. Four more read operations (`get_account_performance`, `get_performance_breakdowns`, `get_equity_timeseries`, `list_deals`) carry to sprint W34 (EPIC-2 stays `in-progress`)
- Last Version: 0.5.0 — the first tool with a path parameter. `list_account_strategies` reads `GET /api/v1/accounts/{accountId}/strategies` and returns the strategies (expert advisors) currently deployed on one MT5 account — the user-scoped sibling of `list_strategies`'s platform-wide catalog. Routes through `accountPath` (US-2.4): every segment is format-validated and `encodeURIComponent`-ed before being joined into a URL, and rejects a traversal attempt before any HTTP call is made. Registers through `registerReadTool` under `strategies:read`; a `404` carries the login/id hint
