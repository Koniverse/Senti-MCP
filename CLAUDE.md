...
- Active Stories: ✅ US-2.4 tool substrate and directory layout — closed, ships 0.2.0. ✅ US-2.5 `list_brokers` — closed, ships 0.3.0. ✅ US-2.6 `list_strategies` — closed, ships 0.4.0. Open: US-2.7 `list_account_strategies`, US-2.8 `list_positions`, US-2.9 `list_pending_orders`
- Next Up: US-2.7 `list_account_strategies` — the first tool that routes through `accountPath` and the `404` login/id hint, per the [read-tool expansion spec](docs/superpowers/specs/2026-08-05-senti-read-tools-expansion-design.md). Its description explicitly contrasts itself against US-2.6's platform-wide catalog framing. Four more read operations (`get_account_performance`, `get_performance_breakdowns`, `get_equity_timeseries`, `list_deals`) carry to sprint W34 (EPIC-2 stays `in-progress`)
- Last Version: 0.4.0 — second tool on the US-2.4 substrate. `list_strategies` reads `GET /api/v1/strategies` and returns the platform-wide catalog of strategies (expert advisors) available to deploy — supported symbols, timeframes, rating, presets — explicitly not what is currently running on an account, which is `list_account_strategies`. No path parameter; registers through `registerReadTool` with no new helper code
